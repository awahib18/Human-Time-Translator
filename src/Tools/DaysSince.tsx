...
export default function DaysSince() {
  ...
  const getLabel = (output: string) => {
    if (output.includes('year')) return 'Years Since';
    if (output.includes('month')) return 'Months Since';
    if (output.includes('week')) return 'Weeks Since';
    if (output.includes('day')) return 'Days Since';
    if (output.includes('hour')) return 'Hours Since';
    if (output.includes('minute')) return 'Minutes Since';
    ...
    <div className="flex flex-col items-center justify-center p-6 md:p-12 w-full h-full">
      ...
      >
        <div className="flex items-center gap-4 mb-2">
          <span className="text-[10px] font-mono text-indigo-400">[ PAST_EVENT_DATE ]</span>
        ...
        <div className="relative">
          <input 
            ...
            max={new Date().toISOString().split('T')[0]} // limit to past dates visually
          />
        </div>
      </motion.div>
